...
*)echo Nginx no esta activo, lo arrancamos; service nginx start;;
esac
echo Pulsa Enter para continuar
read nada
}
...
function workers() {
echo sed -i 's/workers_processes\ 4/worker_processes\ 10/g' "/etc/nginx/nginx.conf" > /tmp/atjobs.txt
...
echo "2) Muestra ocupacion de todos los directorios de /"
echo "3) Subir ligeramente la prioridad a todos los procesos del usuario alumno"
echo "4) Comprobar si esta instalado el servidor el servidor web nginx y si no instalarlo"
echo "5) Comprobar si el servidor web nginx esta arrancando y respondiendo"
echo "6) Programar la reconfiguracion del servidor nginx en la madrigada del domingo al lunes"
echo "7) Mostrar cada 3 segundos todos los procesos del usuario que corre nginx y los 5 que mas memoria real consumen del usuario alumno, orenados por memoria real (rss) descendente (mostrar uid,oid,cmd y rss)"
echo "8) Salir"
echo "=========================================================================================="
...
case $opcion in
	...
	*)echo "Opcion no valida, elige una opcion entre 1 y 8";
	echo "Pulse Enter para continuar"; read nada;;
